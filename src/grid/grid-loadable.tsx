...
export interface CellPropsExt<T = Object> extends CellProps {
  data: string;
  rowData: T;
}
...
export class GridLoadable extends React.Component<Props, State> {
  ...
  renderCell = (props: CellProps) => {
    const { cells, bunchIdx, rowData } = this.state.model.getCells(props.row);
...
export class GridModel extends Publisher {
  ...
  getCells(rowIdx: number): { cells: Array<string>, bunchIdx: number, rowData: Object } {
    ...
    if (!bunch || !bunch.rows || bunch.rows[rowIdx] == null)
      return { cells: null, bunchIdx, rowData: null };

    return { cells: bunch.rows[rowIdx].cells, bunchIdx, rowData: bunch.rows[rowIdx].row };
  }
  ...
  reloadBunch(args: { bunchIdx: number, cancel?: boolean }) {
    ...
    .then(rows => {
      ...
      bunch.rows = [];
      for (let row of rows)
        bunch.rows.push( { cells: Object.keys(row).map(v => row[v]), row } );
